...
function abus_adminbar_output() {
	...
	if( is_admin_bar_showing() ) {
		...
		if( current_user_can( apply_filters( 'abus_switch_to_capability', 'edit_users' ) ) ) {
			...
			$form = '
					<form method="post" action="abus_user_search" id="adminbarsearch">
	...
	if ( ! empty( $user_query->results ) ) {
		...
		foreach ( $user_query->results as $user ) {
			...
			if ( $link ) {
				...
				echo '<li id="switch-to-user-' . $user->ID . '" class="switch-to-user"><a href="' . esc_url( $link, $user ) . '">' . $user->display_name . '</a></li>';
			}
...
function abus_styles() {
	...
	$styles = '
		<style type="text/css">
			...
			#wp-admin-bar-abus_switch_to_user #adminbarsearch::before { content: ""; display:none; }
		</style>
	';
